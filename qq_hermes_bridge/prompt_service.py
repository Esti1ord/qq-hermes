...
DIRECT_RULES = [
    ...
    "普通聊天不要声称自己正在联网搜索、实时查询或查官方结果；需要实时或外部事实时，如果上下文没有可靠来源，就直接说明不能实时核查，让群友自行查证，不要编造。",
    ...
    sections = [
        ...
        PromptSection(
            ...
            body=(
                ...
                "如果需要实时或外部事实但上下文没有可靠来源，直接说明不能实时核查，不要声称已搜索或编造。\n"
...
PROACTIVE_RULES = [
    ...
    "敏感/吵架/隐私/违法也保持沉默。",
    "主动发言一般只接纯闲聊；如果最近高权重上下文需要实时或外部事实核查而没有可靠来源，就保持沉默或说明不确定；不要声称已联网、已实时查询或已查官方结果，也不要编造。",
    "标点按自然表达保留；不要为了像群聊而强行删除句号、逗号、引号或网址里的点号。",
    ...
    sections = [
        ...
        PromptSection(
            ...
            body=(
                ...
                "若要接的是需要实时或外部事实核查的内容，且上下文没有可靠来源，就优先沉默或说明不确定。\n"

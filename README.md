...
PRIMARY_CHAT_MODEL_PROVIDER=custom
PRIMARY_CHAT_MODEL=deepseekv4flash
PRIMARY_CHAT_MODEL_BASE_URL=<custom supplier base url or chat/completions endpoint>
PRIMARY_CHAT_MODEL_API_KEY_ENV=PRIMARY_CHAT_MODEL_API
PRIMARY_CHAT_MODEL_API=<api key, keep in local .env/systemd only>
...
Direct 回复还可以单独配置 fast/strong lane 覆盖：`DIRECT_FAST_MODEL_ALIAS` 会作为普通 direct 回复的模型覆盖；`DIRECT_STRONG_MODEL_ALIAS` 只在更需要稳态推理的 direct 场景作为模型覆盖使用，包括用户回复机器人消息、当前或被引用消息包含图片/OCR 上下文。strong lane 是 **model-only** 覆盖：provider、provider base URL、API-key env 名、Hermes group session、fallback 身份、timeout 和输出上限都沿用当前 direct 生效配置。`DIRECT_MODEL_TIMEOUT_SECONDS` 和 `DIRECT_MAX_OUTPUT_CHARS` 只覆盖 direct 模型超时和回复输出上限；需要 direct 走独立 OpenAI-compatible 通道时再配置 `DIRECT_CHAT_MODEL_PROVIDER`、`DIRECT_CHAT_MODEL_BASE_URL` 或 `DIRECT_CHAT_MODEL_URL`，以及 `DIRECT_CHAT_MODEL_API_KEY_ENV`（或本地 `.env` 里的 raw key 别名）。这些值为空或 `0` 时完全继承普通主文本路由，不影响 proactive、摘要、命令、OCR 或其它 Hermes 调用；日志/metrics/admin state 只记录是否配置、profile、timeout 和输出上限等安全元数据，不记录模型名、provider URL、API-key env 名、prompt 或输出。

修改 `.env` 后通常需要重启 bridge 才会生效。Direct 回复默认按消息逐条排队；如需在高频同一群友连续 @ 时减少重复模型调用，可把 `DIRECT_COALESCE_WINDOW_MS` 从默认 `0` 调成很小的毫秒窗口。该开关只合并尚未开始处理、同群、同发送者、同 direct 路由的普通文本 direct intent；回复引用、图片/OCR、命令、proactive、不同发送者、不同路由或已开始处理的 direct intent 都不会合并。合并后的多条文本只进入 prompt，最终 QQ reply 指向最新一条消息；运行统计/metrics 只记录合并计数、窗口和 `coalesced` 状态等安全元数据，不记录合并文本。设回 `0` 即可回滚。
...
当主文本模型 provider 停服、模型不可用、Hermes/HTTP 调用失败或返回空输出时，Bridge 默认会尝试一次 fallback 文本模型。默认 fallback 是 `VICE_CHAT_MODEL_PROVIDER=deepseek`、`VICE_CHAT_MODEL=deepseekv4flash`，也可以给 fallback 配置 `VICE_CHAT_MODEL_BASE_URL` / `VICE_CHAT_MODEL_URL` 与 `VICE_CHAT_MODEL_API_KEY_ENV` / `VICE_CHAT_MODEL_API` 来走 OpenAI-compatible direct HTTP。旧的 `HERMES_FALLBACK_PROVIDER` / `HERMES_FALLBACK_MODEL` 仍兼容但不再作为推荐示例。fallback 调用不使用群聊持久 session，并且如果 fallback 与当前生效的主模型/provider/base-url/API-env 在归一化后完全相同会自动跳过，避免重复请求同一个下游。旧 provider 标签 `官方` 会归一化为 Hermes provider `deepseek`。
...
PRIMARY_OCR_MODEL_PROVIDER=custom
PRIMARY_OCR_MODEL=mimo
PRIMARY_OCR_MODEL_BASE_URL=<provider base url or chat/completions endpoint>
PRIMARY_OCR_MODEL_API_KEY_ENV=PRIMARY_OCR_MODEL_API
PRIMARY_OCR_MODEL_API=<api key, keep in local .env/systemd only>
OCR_FALLBACK_ENABLED=true
VICE_OCR_MODEL_PROVIDER=custom
VICE_OCR_MODEL=gpt-5.4
VICE_OCR_MODEL_BASE_URL=<fallback provider base url or chat/completions endpoint>
VICE_OCR_MODEL_API_KEY_ENV=VICE_OCR_MODEL_API
VICE_OCR_MODEL_API=<fallback api key, keep in local .env/systemd only>
OCR_EXTERNAL_PROVIDER_ALLOWED=true
```

`PRIMARY_OCR_MODEL_API_KEY_ENV` / `VICE_OCR_MODEL_API_KEY_ENV` 配的是“环境变量名”，不是 key 本身。例如可以把它设为 `VISION_API_KEY`，再由 shell 或 systemd EnvironmentFile 提供 `VISION_API_KEY=<api key>`。raw alias `PRIMARY_OCR_MODEL_API_KEY` / `PRIMARY_OCR_MODEL_API` / `VICE_OCR_MODEL_API_KEY` / `VICE_OCR_MODEL_API` 也按“env var name”处理。主 OCR 还支持本机别名 `IMAGE_MODEL_PROVIDER` / `IMAGE_MODEL` / `IMAGE_MODEL_BASE_URL` / `IMAGE_MODEL_URL` / `IMAGE_MODEL_API_KEY_ENV` / `IMAGE_MODEL_API_KEY` / `IMAGE_MODEL_API`。旧键 `OCR_API_KEY_ENV` / `OCR_FALLBACK_API_KEY_ENV` 仍兼容。不要把真实 key、provider base URL 中的私密部分写进 README、issue、聊天或提交记录。

`PRIMARY_OCR_MODEL_BASE_URL`、`PRIMARY_OCR_MODEL_URL`、`IMAGE_MODEL_BASE_URL`、`IMAGE_MODEL_URL`、`VICE_OCR_MODEL_BASE_URL`、`VICE_OCR_MODEL_URL` 或旧 `OCR_PROVIDER_BASE_URL` 可以是 provider 根路径，也可以已经是 `/chat/completions` endpoint；Bridge 会规范化为 OpenAI-compatible `/chat/completions` 请求。请求会把下载后的图片 bytes 编成 base64 data URL 放进 `messages[0].content`，同时发送 `OCR_IMAGE_PROMPT` 文本。当前支持这些 provider 名称别名：
...
# Optional local aliases for the primary OCR block.

...
OCR_PROVIDER=model
OCR_MODEL=<vision-capable-model>
OCR_PROVIDER_BASE_URL=<provider base url or chat/completions endpoint>
OCR_API_KEY_ENV=<env var containing API key>
OCR_EXTERNAL_PROVIDER_ALLOWED=true
```

`OCR_API_KEY_ENV` 配的是“环境变量名”，不是 key 本身。例如可以把它设为 `VISION_API_KEY`，再由 shell 或 systemd EnvironmentFile 提供 `VISION_API_KEY=<api key>`。不要把真实 key 写进 README、issue、聊天或提交记录。
...
OCR_TRIGGER_MODE=direct_only
OCR_PROVIDER=model
# Fill these only when enabling OCR_PROVIDER=model. OCR_API_KEY_ENV is an env var name, not the key itself.
OCR_MODEL=
...
Bridge 主要有三类本地日志，默认都在 `logs/` 下。不要提交或公开分享这些文件。

### `logs/runtime_stats.jsonl`

这是用于排查性能和稳定性的内容安全运行统计。设计上不记录群聊正文、prompt、模型输出、OCR 文本、图片 URL、token、cookie、API key、完整 provider 响应或完整接口错误正文。
...
每行都是 JSONL，外层含 `ts`，内层 `event` 里通常有 `stat`、`duration_ms`、`duration_bucket`、`status`、`ok`、`error`、`provider`、`result_len_bucket` 等字段。常见 OCR 统计名：

- `ocr_fetch_result`：图片下载是否成功、耗时、content type、bytes 长度；
- `ocr_provider_result`：识图 provider 是否成功、耗时、结果长度桶、安全错误码；
- `ocr_cache_event`：OCR 缓存命中或 inflight 合并；
- `ocr_route_result`：本次路由识图数量和成功数量。
...
这是运行事件日志，会记录路由、下载、识图、发送等事件摘要。为保持内容安全，Bridge 不会把原始 OCR 文本、完整图片 URL、API key、token 或完整 provider 响应写进普通运行日志。`OCR_LOG_TEXT`、`OCR_LOG_IMAGE_URLS` 作为旧配置项保留但不再开启 raw 内容日志。
...
指标由 `qq_hermes_bridge/metrics.py` 直接渲染，不依赖 `prometheus-client`。默认不会导出 `group_id` label，以减少指标标签里的群号暴露；只有在你明确需要按群拆分监控时才设置 `PROMETHEUS_INCLUDE_GROUP_ID_LABEL=true`。指标同样遵循内容安全原则：只导出路由、状态、组件、耗时、队列深度等低基数字段，不导出消息正文、prompt、模型输出、用户标识、token、URL、OCR 文本、API key 或完整 provider 响应。

...
`hermes_fallback_provider`, `ocr_api_key_env`, and
...
|---|---|---|
| Primary text model | `PRIMARY_CHAT_MODEL` → `HERMES_MODEL` | Default groups use this resolved value unless `HERMES_MODEL_BY_GROUP[group_id]` overrides it |
| Primary text provider | `PRIMARY_CHAT_MODEL_PROVIDER` → `HERMES_PROVIDER` | Must be a Hermes-known provider identifier for `hermes chat --provider` |
...
| Fallback text provider | `VICE_CHAT_MODEL_PROVIDER` → `HERMES_FALLBACK_PROVIDER` → `官方` | Must also be a Hermes-known provider identifier |
...
| Setting | Resolution order | Contract |
|---|---|---|
| Primary OCR provider | `PRIMARY_OCR_MODEL_PROVIDER` → `OCR_PROVIDER` → `hermes` | Legacy `OCR_*` names remain compatible as the fallback chain |
| Primary OCR model | `PRIMARY_OCR_MODEL` → `OCR_MODEL` | Used for model-backed OCR providers; builders may fall back to `HERMES_MODEL` when required |
| Primary OCR URL | `PRIMARY_OCR_MODEL_URL` → `PRIMARY_OCR_MODEL_BASE_URL` → `OCR_PROVIDER_BASE_URL` | Provider root or `/chat/completions` endpoint; never log it |
| Primary OCR API key env name | raw-name detection on `PRIMARY_OCR_MODEL_API_KEY` / `PRIMARY_OCR_MODEL_API` / `OCR_API_KEY`, else `PRIMARY_OCR_MODEL_API_KEY_ENV` → `OCR_API_KEY_ENV` | Pass the env var name into the provider; do not copy or log the secret value |
| Fallback OCR provider | `VICE_OCR_MODEL_PROVIDER` → `OCR_FALLBACK_PROVIDER` → `model` | Used only after non-`ok` primary OCR result and only when external providers are allowed |
...
| Fallback OCR API key env name | raw-name detection on `VICE_OCR_MODEL_API_KEY` / `VICE_OCR_MODEL_API` / `OCR_FALLBACK_API_KEY`, else `VICE_OCR_MODEL_API_KEY_ENV` → `OCR_FALLBACK_API_KEY_ENV` | Pass the env var name into the provider; do not copy or log the secret value |
...
- Hermes CLI provider names must be Hermes-known identifiers. Display/vendor
  labels that Hermes does not know are configuration errors: they cause a
  nonzero Hermes run, may trigger fallback, and must not be documented as valid
  active `*_PROVIDER` values unless Hermes is configured to recognize them.
- `run_hermes_raw_result()` attempts the active primary command first. Fallback
  is attempted when the primary subprocess fails to start, times out, returns
  nonzero, or returns empty output.
...
- Text fallback is skipped when the fallback `(model, provider)` matches the
  active primary `(model, provider)` after per-group overrides.
...
| Neither vice alias nor legacy text fallback is set | Use hard-coded text fallback defaults `deepseekv4flash` / `官方` |
...
| Fallback text model/provider equals active per-group primary | Skip fallback to avoid retrying the same outage |
...
- Bad: using a display/vendor label as `PRIMARY_CHAT_MODEL_PROVIDER` or
  `VICE_CHAT_MODEL_PROVIDER` when Hermes CLI does not know that identifier.
...
- `tests/test_config_utils_module.py` must assert:
  ...
  - raw OCR key envs resolve to env var names such as `PRIMARY_OCR_MODEL_API`
    and `VICE_OCR_MODEL_API`, not secret values;
...
- `tests/test_hermes_group_sessions.py` must assert:
  ...
  - matching primary/fallback model-provider skips fallback;
  - Hermes start/error logs do not include prompt/model/provider details.
...
- Syntax and focused validation:
  ...
  ./venv/bin/python -m pytest tests/test_config_utils_module.py tests/test_hermes_group_sessions.py tests/test_bridge_ocr.py tests/test_direct_reply_inflight.py -q
  ```
...
- Using a provider display/vendor label that Hermes CLI does not recognize.
